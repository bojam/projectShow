...
metro.preset = function() {
	...
	$(metro.wrap).css('width', metro.row * metro.gen.w + "px");
};
...
metro.putData = function(list) {
	(function setBig() { //大格子初始化设置
		...
		while (i < bigBox.length) {
			bigBoxCont = $(bigBox[i]).find(".innerBox");
...
metro.sort = function(size) {
	...
	metro.memory = memory;
	$(metro.wrap).css('height', metro.gen.h * metro.maxY + 'px');
